...
const DEFENSE_TICKERS = [
  "PLTR", "LDOS", "SAIC", "CACI", "BAH", "DRS", "KTOS", "RCAT", "AVAV", "HII",
  "CWAN", "BBAI", "SPIR", "TDG", "AXON", "S", "CRWD",
];

const MIN_MARKET_CAP = 2_000_000_000;
const MAX_MARKET_CAP = 50_000_000_000;
const MIN_LIQUIDITY = 5_000_000;
const MIN_AGI_SCORE = 75;
const BATCH_SIZE = 5;
const TOP_N = 8;
...
export async function runDefenseAnalyst(): Promise<CandidateTicker[]> {
  const candidates: CandidateTicker[] = [];

  for (let i = 0; i < DEFENSE_TICKERS.length; i += BATCH_SIZE) {
    const batch = DEFENSE_TICKERS.slice(i, i + BATCH_SIZE);
