import { useMemo } from "react";
import type { Holding } from "@shared/schema";

interface CorrelationMatrixProps {
  holdings: Holding[];
  correlationData?: Record<string, number>;
  top6Tickers?: string[];
}

function getCellColor(val: number): string {
  if (val >= 0.95) return "#00D9FF";
  if (val >= 0.7) return "rgba(0, 217, 255, 0.55)";
  if (val >= 0.4) return "rgba(0, 217, 255, 0.28)";
  if (val >= 0.1) return "rgba(0, 217, 255, 0.12)";
  if (val >= -0.1) return "rgba(255,255,255,0.04)";
  if (val >= -0.4) return "rgba(255, 68, 88, 0.2)";
  return "rgba(255, 68, 88, 0.45)";
}

function getTextColor(val: number): string {
  if (Math.abs(val) >= 0.7) return "#FFFFFF";
  if (Math.abs(val) >= 0.3) return "#C9D1D9";
  return "#8B949E";
}

export function CorrelationMatrix({ holdings, correlationData, top6Tickers }: CorrelationMatrixProps) {
  const tickers = useMemo(() => {
    if (top6Tickers && top6Tickers.length >= 2) return top6Tickers;
    return [...holdings].sort((a, b) => (b.value ?? 0) - (a.value ?? 0)).slice(0, 6).map(h => h.ticker);
  }, [holdings, top6Tickers]);

  const matrix = useMemo(() => {
    if (!correlationData) return null;
    return tickers.map(t1 => tickers.map(t2 => correlationData[`${t1}_${t2}`] ?? (t1 === t2 ? 1.0 : 0)));
  }, [tickers, correlationData]);

  if (tickers.length < 2) {
    return (
      <div style={{ display: "flex", flexDirection: "column", flex: "1 1 0", minHeight: 0, background: "#0B0F1A" }}>
        <div className="terminal-panel-header" style={{ flexShrink: 0 }}>
          <span className="terminal-panel-title">Correlation Matrix</span>
          <span className="terminal-badge">PRICE-BASED</span>
        </div>
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 8 }}>
          <span style={{ fontSize: 9, color: "#8B949E" }}>Need 2+ holdings</span>
        </div>
      </div>
    );
  }

  if (!matrix) {
    return (
      <div style={{ display: "flex", flexDirection: "column", flex: "1 1 0", minHeight: 0, background: "#0B0F1A" }}>
        <div className="terminal-panel-header" style={{ flexShrink: 0 }}>
          <span className="terminal-panel-title">Correlation Matrix</span>
          <span className="terminal-badge">PRICE-BASED</span>
        </div>
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 8 }}>
          <span style={{ fontSize: 9, color: "#8B949E" }}>Loading price data...</span>
        </div>
      </div>
    );
  }

  return (
    <div style={{ display: "flex", flexDirection: "column", flex: "1 1 0", minHeight: 0, background: "#0B0F1A" }}>
      <div className="terminal-panel-header" style={{ flexShrink: 0 }}>
        <span className="terminal-panel-title">Correlation Matrix</span>
        <span className="terminal-badge">PRICE-BASED</span>
      </div>
      <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", minHeight: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: 4 }}>
        <table style={{ borderCollapse: "collapse" }}>
          <thead>
            <tr>
              <th style={{ width: 36 }} />
              {tickers.map((t) => (
                <th key={t} style={{ fontSize: 10, fontWeight: 600, color: "#8B949E", letterSpacing: 0.5, padding: "2px 1px", textAlign: "center", fontFamily: "'JetBrains Mono', monospace", width: 38 }}>
                  {t}
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {tickers.map((rowT, ri) => (
              <tr key={rowT}>
                <td style={{ fontSize: 10, fontWeight: 600, color: "#8B949E", letterSpacing: 0.5, padding: "1px 4px 1px 0", textAlign: "right", fontFamily: "'JetBrains Mono', monospace" }}>
                  {rowT}
                </td>
                {matrix[ri].map((val, ci) => (
                  <td key={ci} className="font-mono tabular-nums" style={{ fontSize: 10, fontWeight: 500, color: getTextColor(val), background: getCellColor(val), textAlign: "center", padding: "3px 1px", border: "1px solid #0B0F1A", width: 38, height: 24 }}>
                    {val.toFixed(2)}
                  </td>
                ))}
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}
